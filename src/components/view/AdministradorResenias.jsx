...
import { eliminarResenias, listarJuegos, listaresenias } from '../helpers/queries';
...
function AdministradorResenias() {
  ...
  useEffect(() => {
    ...
    resenias.map((resenia)=> {
      juegos.map((juego)=>{
        if(resenia.idUsuario === parseInt(idUsuarioActivo) && juego.id === resenia.idJuego)
        {
          Datos = {
  ...
  <ItemResenia key={dato.idResenia} dato={dato} {...dato} handleEliminarClick={handleEliminarClick} />
))}
        </tbody>
      </Table>
        </section>
